...
export default function OtherProfile() {
  ...
  useEffect(() => {
    ...
    const userData = AllUserData.find((d) => d.user_id === userID);
    if (userData) setUserProfile(userData);

    setUserPosts(BlogData.filter((d) => d.user.user_id === userID));
    setUserHistory(
      BlogData.filter(
        (d) => d.info.views_count.includes(userID) && d.user.user_id !== userID
      )
    );
    setUserLiked(
      BlogData.filter(
        (d) => d.info.like_count.includes(userID) && d.user.user_id !== userID
      )
    );
    setLoading(false);
  }, [userID]);
  ...
  if (!userProfile.user_id) return <SideBar><p className="text-center text-gray-500 py-12">User not found</p></SideBar>;

  const isOwn = userProfile.user_id === userAcc.user_id;

  return (
    <SideBar>
      <h1 className="text-2xl font-bold text-gray-900 mb-6">
        {isOwn ? "Your" : `${userProfile.username}'s`} Profile
      </h1>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6 mb-8">
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5">
          ...
          <div className="flex-1 text-center sm:text-left">
            ...
            <p className="text-sm text-gray-500 mt-1">{userProfile.followers.length} followers</p>
            <p className="text-sm text-gray-600 mt-1">{userProfile.user_description}</p>
            ...
            <div className="flex items-center justify-center sm:justify-start gap-3 mt-4">
              {userProfile.socials?.github && (
                <Link href={userProfile.socials.github} className="text-gray-400 hover:text-gray-600 text-sm">GitHub</Link>
              )}
              {userProfile.socials?.twitter && (
                <Link href={userProfile.socials.twitter} className="text-gray-400 hover:text-gray-600 text-sm">Twitter</Link>
              )}
            </div>
          </div>
          <button
          ...
          >
            {isOwn ? "Edit" : userProfile.followers.includes(userAcc.user_id) ? "Following" : "Follow"}
          </button>
        </div>
      </div>
